...
def sign_up():
    if request.method=="POST":
        ...
        if User.query.filter(User.email==user_email).first() is not None:
            ...
            new_user = User(name=user_name, email=user_email,designation=designation,department=department, password=generate_password_hash(user_password1, method='sha256'))
